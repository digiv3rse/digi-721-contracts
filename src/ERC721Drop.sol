...
/**
 * @notice DIGI NFT Base contract for Drops and Editions
 *
    ...
    ERC721RewardsStorageV1
{
    ...
    /// @dev DIGI V3 transfer helper address for auto-approval
    ...
    /// @notice DiGi Mint Fee
    uint256 private immutable DIGI_MINT_FEE;

    /// @notice Mint Fee Recipient
    address payable private immutable DIGI_MINT_FEE_RECIPIENT;
    ...
    constructor(
        ...
        DIGI_MINT_FEE = _mintFeeAmount;
        DIGI_MINT_FEE_RECIPIENT = _mintFeeRecipient;
    }
    ...
    /// @dev Setup auto-approval for DiGi v3 access to sell NFT
    ...
    /// @notice DIGI fee is fixed now per mint
    ...
    {
        recipient = DIGI_MINT_FEE_RECIPIENT;
        fee = DIGI_MINT_FEE * quantity;
    }
    ...
    function _payoutDiGiFee(uint256 quantity) internal {
        // Transfer DIGI fee to recipient
        (, uint256 zoraFee) = zoraFeeForAmount(quantity);
        (bool success, ) = DIGI_MINT_FEE_RECIPIENT.call{value: zoraFee, gas: FUNDS_SEND_GAS_LIMIT}(
            ""
        );
        emit MintFeePayout(zoraFee, DIGI_MINT_FEE_RECIPIENT, success);
    }
