...
const getAddresses = () => {
  ...
  for (const addressesFile of addressesFiles) {
    const jsonAddress = JSON.parse(
      ...
      DIGI_NFT_CREATOR_PROXY: Address;
      DIGI_NFT_CREATOR_V1_IMPL: Address;
    };
    ...
    addAddress(
      'DiGiNFTCreatorV1',
      chainId,
      jsonAddress.DIGI_NFT_CREATOR_PROXY
    );
    // addAddress("DiGiNFTCreatorV1", chainId, jsonAddress.DIGI_NFT_CREATOR_V1_IMPL);
  }
