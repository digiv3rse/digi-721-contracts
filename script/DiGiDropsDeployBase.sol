...
abstract contract DiGiDropsDeployBase is ScriptDeploymentConfig {
    ...
    function getDeploymentJSON(DropDeployment memory deployment) internal returns (string memory deploymentJson) {
        ...
        vm.serializeAddress(deploymentJsonKey, DIGI_NFT_CREATOR_PROXY, deployment.factory);
        // Get the JSON key as a seralized string
        deploymentJson = vm.serializeAddress(deploymentJsonKey, DIGI_NFT_CREATOR_V1_IMPL, deployment.factoryImpl);
    ...
    function deployTestContractForVerification(DiGiNFTCreatorV1 factory, address deployer) internal {
